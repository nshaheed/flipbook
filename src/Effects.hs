...
aniActive2 d a = clamp . stretchTo d . mkActive 0 1 $ \t -> a t

-- :: Duraction -> a -> Active a
fadein, fadeout :: Duration -> Canvas () -> Active (Canvas())
fadein d c = aniActive2 d $ \t -> saveRestore $ do globalAlpha(fromTime t)
                                                   c
...
translateAniTxt d (x,y) txt disp precede style = aniActive2 d $ \t -> saveRestore $ do font style
...
translateAniCanv (x1,y1) (x2,y2) t c = saveRestore $ do let (dx,dy) = (x2 - x1, y2 - y1)
                                                            t'      = fromTime t
